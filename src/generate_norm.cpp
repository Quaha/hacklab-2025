...
struct Zic{
    ...
    const float x1 = 3.6541528853610088;
    const float A = 4.92867323399e-3; /// area under rectangle

    Zic() {
        // coordinates of the implicit rectangle in base layer
        stairHeight[0] = exp(-0.5 * x1 * x1);
    ...
    float NormalZiggurat(Generator &gen) {
        ...
        do {
            ...
            if (uni_f_distab(gen,stairHeight[stairId - 1], stairHeight[stairId]) < exp(-0.5 * x * x))
                return ((signed)B > 1073741823) ? x : -x;
